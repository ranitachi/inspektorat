					...
					<form action="{{ route('pengguna.store') }}" method="POST">
						...
						<div class="form-group">
							<select name="level" class="form-control" id="level">
								...
								<option value="1">Administrator Website</option>
								<option value="11">Administrator Simulasi Ligitasi</option>
								<option value="12">Ketua Pengadilan</option>
							</select>
						</div>
						<div class="form-group">
							...
							@foreach ($users as $key => $user)
								<tr>
									...
									@switch($user->level)
										...
										@break
										@case(11)
											<td>Administrator Simulasi Ligitasi</td>
										@break
										@case(12)
									...
									<td>
										<a class="btn btn-xs btn-warning btn-edit" data-toggle="modal" data-target="#modalubah" data-value="{{ $user->id }}">
...
@section('footscript')
	<script>
        ...
        $('#table').on('click', '.btn-edit', function(){
            ...
            $.ajax({
                url: "{{ url('backend/pengguna') }}/"+id+"/edit",
                success: function(res) {
					$('#form-update').attr('action', "{{ url('backend/pengguna') }}/"+id)
        ...
        $('#table').on('click', '.btn-delete', function(){
            var id = $(this).data('value')
			$('#form-delete').attr('action', "{{ url('backend/pengguna') }}/"+id)			
        })
	</script>
@endsection